 ...
 */
@SuppressLint("NewApi")
public class iHorizontalListView extends AdapterView<ListAdapter> implements
		ViewTreeObserver.OnTouchModeChangeListener {
	private static final String LOGTAG = "iHorizontalListView";
	...
	public interface OnScrollListener {
		 ...
		 *            {@link #SCROLL_STATE_IDLE}.
		 */
		public void onScrollStateChanged(iHorizontalListView view,
				int scrollState);
		 ...
		 */
		public void onScroll(iHorizontalListView view, int firstVisibleItem,
	 ...
	 * 
	 * @see iHorizontalListView.RecycleBin
	 * @see iHorizontalListView#setRecyclerListener(iHorizontalListView.RecyclerListener)
	 */
	...
	public iHorizontalListView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public iHorizontalListView(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.iHorizontalListView, defStyle, 0);
		try {
			// initializeScrollbars(TypedArray)
			Method initializeScrollbars = android.view.View.class
		...
		mDrawSelectorOnTop = a.getBoolean(
				R.styleable.iHorizontalListView_android_drawSelectorOnTop,
				false);

		Drawable d = a
				.getDrawable(R.styleable.iHorizontalListView_android_listSelector);
		if (d != null) {
			setSelector(d);
		}

		int orientation = a.getInt(
				R.styleable.iHorizontalListView_android_orientation, -1);
		if (orientation >= 0) {
			setOrientation(Orientation.values()[orientation]);
		}

		int choiceMode = a.getInt(
				R.styleable.iHorizontalListView_android_choiceMode, -1);
		if (choiceMode >= 0) {
	 ...
	 * 
	 * @see iHorizontalListView.RecycleBin
	 * @see iHorizontalListView.RecyclerListener
	 */
	...
	public boolean showContextMenuForChild(View originalView) {
		...
		if (longPressPosition >= 0) {
			...
			if (listener != null) {
				handled = listener.onItemLongClick(iHorizontalListView.this,
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			final int index = MotionEventCompat.findPointerIndex(ev,
					mActivePointerId);
			if (index < 0) {
				if(isLog)Log.e(LOGTAG,
						"onInterceptTouchEvent could not find pointer with id "
								+ mActivePointerId
								+ " - did iHorizontalListView receive an inconsistent "
								+ "event stream?");
				return false;
			}
	...
	public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
		...
		event.setClassName(iHorizontalListView.class.getName());
	}
	...
	public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
		...
		info.setClassName(iHorizontalListView.class.getName());
	...
	private int getArrowScrollPreviewLength() {
		// FIXME: iHorizontalListView has no fading edge support just yet but
		// using it
	...
	private void layoutChildren() {
		...
		try {
			...
			if (mItemCount == 0) {
				...
				throw new IllegalStateException(
						"The content of the adapter has changed but "
								+ "iHorizontalListView did not receive a notification. Make sure the content of "
								+ "your adapter is not modified from a background thread, but only "
								+ "from the UI thread. [in iHorizontalListView("
								+ getId() + ", " + getClass()
			...
			if (selected != null) {
				...
				// restore
				// focus (solutions.ismart.libraryismart.i.e. something focusable in touch mode)
	 ...
	 * returns the height with this iHorizontalListView's padding and item
	 * margin heights included. If maxHeight is provided, the measuring will
	 * stop when the current height reaches maxHeight.
	 * 
	 ...
	 * @return The height of this iHorizontalListView with the given children.
	 */
	private int measureHeightOfChildren(int widthMeasureSpec,
		...
		for (i = startPosition; i <= endPosition; ++i) {
			...
			if (returnedHeight >= maxHeight) {
				...
				// then the solutions.ismart.libraryismart.i'th position did not fit completely.
				return (disallowPartialChildPosition >= 0) // Disallowing is
						...
						&& (prevHeightWithoutPartialChild > 0) // We have a prev
																// height
						&& (returnedHeight != maxHeight) // solutions.ismart.libraryismart.i'th child did not
															// fit completely
	 ...
	 * the width with this iHorizontalListView's padding and item margin widths
	 ...
	 *            not be completely shown, so a value of 2 (remember, inclusive)
	 *            would be good (assuming startPosition is 0).
	 * @return The width of this iHorizontalListView with the given children.
	 */
			...
			if (returnedWidth >= maxWidth) {
				...
				return (disallowPartialChildPosition >= 0) // Disallowing is
						...
						&& (prevWidthWithoutPartialChild > 0) // We have a prev
																// width
						&& (returnedWidth != maxWidth) // solutions.ismart.libraryismart.i'th child did not fit
														// completely
	...
	private boolean performLongPress(final View child,
		...
		OnItemLongClickListener listener = getOnItemLongClickListener();
		if (listener != null) {
			handled = listener.onItemLongClick(iHorizontalListView.this, child,
					longPressPosition, longPressId);
		}
		...
		if (!handled) {
			mContextMenuInfo = createContextMenuInfo(child, longPressPosition,
					longPressId);
			handled = super.showContextMenuForChild(iHorizontalListView.this);
		}
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		/**
		 * When a iHorizontalListView is measured with an AT_MOST measure spec,
	...
	private class AdapterDataSetObserver extends DataSetObserver {
		...
		public void onChanged() {
			...
			if (iHorizontalListView.this.mHasStableIds
					&& mInstanceState != null && mOldItemCount == 0
					&& mItemCount > 0) {
				iHorizontalListView.this.onRestoreInstanceState(mInstanceState);
				mInstanceState = null;
			} else {
				rememberSyncState();
			}
		...
		public void onInvalidated() {
			...
			if (iHorizontalListView.this.mHasStableIds) {
				...
				mInstanceState = iHorizontalListView.this.onSaveInstanceState();
			}
	...
	static class SavedState extends BaseSavedState {
		 ...
		 * {@link iHorizontalListView#onSaveInstanceState()}
		 */
		...
		public String toString() {
			return "iHorizontalListView.SavedState{"
	...
	private static class ArrowScrollFocusResult {
		...
		/**
		 * How {@link iHorizontalListView#arrowScrollFocused} returns its
		 * values.
		 */
